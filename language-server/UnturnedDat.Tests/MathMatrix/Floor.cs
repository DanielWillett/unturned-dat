...
{
    [Test]
    public static void CheckInactiveTypesRemoved_Floor([Range(0, InactiveTypeLen - 1)] int typeNum)
    {
        Type testType = typeof(FloorExecuteVisitor<>).MakeGenericType(InactiveTypes[typeNum]);
    ...
    [Test]
    public static void CheckActiveTypesDontRecurse_Floor([Range(0, ActiveTypeLen - 1)] int typeNum)
    {
        Type testType = typeof(FloorExecuteVisitor<>).MakeGenericType(ActiveTypes[typeNum]);
        BaseExecuteVisitor testInstance = (BaseExecuteVisitor)Activator.CreateInstance(testType)!;
    ...
    [TestCase(-95.1d, -96d)]
    [TestCase(-95.9d, -96d)]
    public static void CheckValues_Floor(object arg, object result)
    {
        Type testType = typeof(FloorExecuteVisitor<>).MakeGenericType(arg.GetType());
        BaseExecuteVisitor testInstance = (BaseExecuteVisitor)Activator.CreateInstance(testType)!;
    ...
    [Test]
    public static void CheckValues_Floor_Decimal([Range(0, 6)] int argType)
    {
        ...
        Assert.That(testInstance.Execute(arg), Is.True);
        Assert.That(testInstance.ExecutedCt, Is.EqualTo(1));
        Assert.That(testInstance.Value, Is.EqualTo(Math.Floor(arg)));
    }

    private class FloorExecuteVisitor<TIn> : BaseExecuteVisitor
        where TIn : IEquatable<TIn>
    {
        public override bool Execute()
        {
            FloorExecuteVisitor<TIn> visitor = this;
            return MathMatrix.Floor(default(TIn)!, ref visitor);
        }

        public override bool Execute(params object[] values)
        {
            FloorExecuteVisitor<TIn> visitor = this;
            return MathMatrix.Floor((TIn)values[0], ref visitor);
        }
    }
}