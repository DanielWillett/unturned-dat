...
public class ParserTest_UInt8
{
    ...
    [TestCase("127", (byte)127)]
    public async Task ParseIntegers(string text, byte number)
    {
        ParserTest<byte> test = ParserTest<byte>.CreateFromSingleProperty(text, number, UInt8Type.Instance);
        _disposable = test;
    ...
    {
        ParserTest<byte> test = ParserTest<byte>.CreateFromSinglePropertyExpectFailure(text, UInt8Type.Instance);
